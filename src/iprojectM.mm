...
void initProjectM( VisualPluginData * visualPluginData, std::string presetPath ) {
    // Create projectM instance using the new API
    ...
    // Set projectM parameters using the new API
    projectm_set_mesh_size(pm, 140, 110);
    projectm_set_fps(pm, 60);
    projectm_set_soft_cut_duration(pm, 2.0); // seconds
    projectm_set_preset_duration(pm, 30.0); // seconds
    projectm_set_beat_sensitivity(pm, 3.0);
    ...
    // Note: In the new API, presets are loaded on demand rather than selecting a random preset at initialization
